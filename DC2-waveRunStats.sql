...
as time,count(distinct c.sku_id)

from wmosp.pkt_hdr_intrnl a, wmosp.ship_wave_parm b,(select pkt_ctrl_nbr, pkt_qty, sku_id from wmosf.pkt_dtl) c,
...
from (SELECT ref_value_1, 'S' type, min(log_date_time) timev from wmosf.msg_log
                 where ref_value_4 = 'H' and ref_code_1 = '19' and msg like 'Wave process started%' group by ref_value_1
                  union SELECT ref_value_1, 'E' type, min(log_date_time) timev from wmosp.msg_log
                   where ref_value_4 = 'H' and ref_code_1 = '19' and msg like 'Wave process completed%' group by ref_value_1
     ) group by ref_value_1) where start_time <> end_time) d, wmosp.wave_parm e
